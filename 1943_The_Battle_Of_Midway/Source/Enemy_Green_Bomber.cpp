...
void Enemy_Green_Bomber::Update()
{
	...
	if (this->health > 0)
	{
		...
		{
			if(timer == fase[2]) App->particles->AddParticle(App->particles->enemyShot, position.x + 50, position.y + 75, Collider::ENEMY_SHOT, true);
			downAnim.Reset();
			currentAnim = &stayAnim;
		}
		if (timer >= fase[3] && timer < fase[4])
		{
			if(timer == fase[3]) App->particles->AddParticle(App->particles->enemyShot, position.x + 50, position.y + 75, Collider::ENEMY_SHOT, true);
			stayAnim.Reset();
			currentAnim = &upAnim;
			invincible = false;
		...
		if (timer == fase[4])
		{
			App->particles->AddParticle(App->particles->enemyShot, position.x + 50, position.y + 75, Collider::ENEMY_SHOT, true);
		...
		{
			if(timer == fase[6]) App->particles->AddParticle(App->particles->enemyShot, position.x + 50, position.y + 75, Collider::ENEMY_SHOT, true);
			downAnim.Reset();
			currentAnim = &stayAnim;
		}
		if (timer >= fase[7] && timer < fase[8])
		{
			if(timer == fase[7])  App->particles->AddParticle(App->particles->enemyShot, position.x + 50, position.y + 75, Collider::ENEMY_SHOT, true);
			stayAnim.Reset();
			currentAnim = &upAnim;
			invincible = false;
			//collider->type = Collider::Type::ENEMY;
